...
Page({
    ...
    for (const date in bookingsByDate) {
      ...
      if (totalPeople >= 4) {
        ...
        activities.push({
          ...
          description: activity?.description || '待定',
          date: date,
  ...
  // 提交预约（云数据库版）
  async submitBooking() {
    const { bookingDate, selectedTimeSlot, bookingPeople } = this.data;
    const userInfo = wx.getStorageSync('userInfo');
    ...
    const bookingName = userInfo.name || userInfo.nickName || '游客';
    const bookingUserId = userInfo._id || '';
    
    console.log('用户预约:', { phone: bookingPhone, name: bookingName, userId: bookingUserId });
    
    if (!selectedTimeSlot) {
      wx.showToast({ title: '请选择时间段', icon: 'none' });
      return;
    }
    if (!bookingPhone || bookingPhone.length < 11) {
      wx.showToast({ title: '请先登录', icon: 'none' });
      return;
    }
